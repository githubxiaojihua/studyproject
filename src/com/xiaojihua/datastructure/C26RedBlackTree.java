 ...
 *
 * 先看课本和笔记，了解实现原理，否则看不懂这里面的逻辑
 * @author Mark Allen Weiss
 */
public class C26RedBlackTree<AnyType extends Comparable<? super AnyType>> {
    ...
    private RedBlackNode<AnyType> current;//最终指向当前节点
    private RedBlackNode<AnyType> parent;//指向当前节点的父节点
    private RedBlackNode<AnyType> grand;//指向当前节点的祖父
    private RedBlackNode<AnyType> great;//指向当前节点的曾祖

    /**
     * Construct the tree.
     */
    public C26RedBlackTree( )
    {
    ...
    private void handleReorient( AnyType item )
    {
        ...
        if( parent.color == RED )   // 调用后如果出现冲突则满足条件则进行旋转
        {
            grand.color = RED;
            if( ( compare( item, grand ) < 0 ) !=
                    ( compare( item, parent ) < 0 ) )
                parent = rotate( item, grand );  // 执行双旋转（执行第一次旋转）
            current = rotate( item, great );//执行单旋转（如果双旋转则此句代表双循环中的第二个循环。
            current.color = BLACK;
        }
        header.right.color = BLACK; // 防止颜色翻转和旋转导致根节点的颜色变成红色
    }
     ...
     * @param item handleReorient传过来的元素.
     * @param parent 一直指向要进行旋转的子树的父节点.
    ...
    {
        C26RedBlackTree<Integer> t = new C26RedBlackTree<>( );
